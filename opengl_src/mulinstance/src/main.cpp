...
public:
	EXFrameBufferRenderAttach(std::string ex_name):ExampleTemplate(ex_name) {
		...
		cube_ = std::make_shared<Cube>(std::make_unique<Texture2D>(FileSystem::getPath("image/guaishao.jpg")));
	}
	...
	virtual void render() override {
		...
		cube_->set_model_matrix(glm::mat4(1.0f));
		cube_->Render(ShaderLoader::GetShaderProgram("tbasic"));

		//glBindFramebuffer(GL_FRAMEBUFFER, 0); // 返回默认
		//glClearColor(1.0f, 1.0f, 1.0f, 1.0f);
		//glClear(GL_COLOR_BUFFER_BIT);
		//ShaderLoader::GetShaderProgram("frame_blur").use();
	...
	virtual void init() override {
		...
		texture_ = std::make_unique<Texture2D>(WIN_WIDTH, WIN_HEIGHT, GL_RGB);
