...
void main(){
	...
	vec3 ambient_influence = ambient_strength * uLightColor * uMaterial.ambient_color;

	//计算漫反射光
	float diffuse_strength = 0.5f;
	vec3 diffuse_influence = max(dot(normal, light_direction), 0.0f) * diffuse_strength * uLightColor * uMaterial.diffuse_color;

	//计算镜面反射
	float specular_strength = 0.8f;
	vec3 specular_influence = pow(max(dot(reflect_direction , camera_direction), 0.0f), 32) * 
									specular_strength * uLightColor * uMaterial.specular_color;
